function output = fft_1D(input)
    ...
    %and recursive two array
        ...
        f_odd = input(:,2:2:n);
        %recursive
        f_even_out = fft_1D(f_even);
        f_odd_out = fft_1D(f_odd);
        ...
        for j = 1:k
            ...
            output(:,j) = f_even_out(:,j) + f_odd_out(:,j) * complex;
            output(:,j+k) = f_even_out(:,j) - f_odd_out(:,j) * complex;
        end
    end
end